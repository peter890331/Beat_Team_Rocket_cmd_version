    ...
    if not hwnd:
        ...
        try:
            ...
            if not os.path.exists(NEMO_DIR):
                print(f"* 錯誤: 找不到 NemoADB 資料夾於 {NEMO_DIR}")
                return None
            os.chdir(NEMO_DIR)
...
def get_moonani_targets(pkm_types=[]):
    ...
    if not pkm_types:
        ...
        try:
            ...
            for r in rows:
                if any(b.lower() in r.lower() for b in blacklist):
                    continue
                m = re.search(r'(-?\d+\.\d+)\s*,\s*(-?\d+\.\d+)', r)
                if m:
                    all_targets.append({"lat": float(m.group(1)), "lng": float(m.group(2))})
            return all_targets
        except:
            return []
    else:
...
def load_json(f, d):
    return json.load(open(f, 'r')) if os.path.exists(f) else d
            ...
            if move_location(lat, lng):
                ...
                for _ in range(2):
                    ...
                    if find_image("close_button.png", 0.85) or find_image("battle.png", 0.8):
                        entered = True
                        break
                    if not find_image("map.png", 0.90):
                        adb_back()
                        time.sleep(2)
                ...
                if battle_ready:
                    ...
                    t_thread = threading.Thread(target=attack_worker)
                    t_thread.start()
                    try:
